...
const ScheduleManagement = () => {
  ...
  const handleLeaveSubmit = (formData) => {
    ...
    const scheduleOptions = [
      ...
      {
        id: 2,
        name: "Công nghệ phần mềm-2-24 (CSE481_002) - Thứ 2, Tiết 10-12",
      },
  ...
  const handleMakeupSubmit = (formData) => {
    ...
    const selectedSchedule = missedSchedules.find(
      (s) => s.id == formData.originalScheduleId
    );
    const scheduleName = selectedSchedule
      ? selectedSchedule.name.split(" - ")[0]
      : "Lớp học phần";
    ...
    const missedDateMatch = selectedSchedule
      ? selectedSchedule.name.match(/(\d{1,2}\/\d{1,2}\/\d{4})/)
      : null;
    ...
    const newSchedule = {
      ...
      time:
        timeMapping[formData.makeupTime] || "Tiết 7 → Tiết 9 (12:55 - 15:35)",
      note: `Bù cho buổi nghỉ ngày ${missedDate}${
